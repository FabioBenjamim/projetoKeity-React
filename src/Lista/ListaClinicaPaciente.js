...
const Locais = (props) => {
  ...
  const locais = props.clinicas.map((local) => {
    var dist =
      6371 *
      Math.acos(
        Math.cos(toRadians(-23.526)) *
          Math.cos(toRadians(local.lat)) *
          Math.cos(toRadians(local.lng) - toRadians(-46.6388)) +
          Math.sin(toRadians(-23.526)) * Math.sin(toRadians(local.lat))
      );
    if (props.range >= dist)
      return (
  ...
  buscaPorEspecialização = (event) => {
    ...
    ApiService.BuscaPorEspecializade(this.state.especialidade)
      ...
      .then((res) => {
        res.map((x) => {
          this.setState({
            clinicas: [...this.state.clinicas, ...x.consultorios],
          });
        });
      });
  };
